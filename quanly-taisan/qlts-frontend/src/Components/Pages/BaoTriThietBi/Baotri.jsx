...
class Baotri extends Component {
    ...
    handleCapnhat = async () => {
        let obj = {
            maPhieuBaoTri: null,
            ngayBaoTri: null,
            maNhanVienChiuTrachNhiem: null,
            noiBaoTri: null,
            diaChiBaoTri: null,
            phiBaoTri: null,
            lstThietBi: this.state.lstThietBi
        }

        var r = window.confirm("Lập danh sách bảo trì cho thiết bị ??");
        if (r === true) {
            await lapDanhSachThietBiBaoTri(obj);
            alert(1)
            history.push('/app/dsphieubaotri');
        }
    }
